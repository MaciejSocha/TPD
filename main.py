...
def print_mini_maks(matrix: list, result: tuple):
    ...
    print("Z punktu widzenia kryterium Walda najlepszą decyzją jest opcja: ", *result[1], sep=' ')
...
def print_maks_maks(matrix: list,result: tuple):
    ...
    for i, name in enumerate(result[0]):
        print("wybór {}: {}".format(i + 1, name))
    return print("Z punktu widzenia kryterium optymistycznego najlepszą decyzją jest opcja: ", *result[1], sep=' ')
...
def print_k_hurwicz(result: tuple, factor: float):
    ...
    for i, name in enumerate(result[0]):
        print("wybór {} : {}".format(i + 1, name))
    print("Z punktu widzenia kryterium Hurwicza najlepszą decyzją dla współczynnika ostrożności {} jest opcja:".format(factor), *result[1], sep=' ')
...
def print_k_bayesa(result: tuple, prob: list):
    ...
    for i, name in enumerate(result[0]):
        print("wybór {} : {}".format(i + 1, name))
    print("Z punktu widzenia kryterium Bayesa Laplace'a najlepszą decyzją dla prawdopodobieństwa ", *prob, sep=' ', end='')
...
def print_k_savage(result: tuple):
    ...
    for i, name in enumerate(result[0]):
        print("wybór {} : {}".format(i + 1, name))
    print("Z punktu widzenia kryterium Savage'a najlepszą decyzją jest opcja ", *result[1], sep=' ')
...
while not exit:
    ...
    if option == '1':
        print_mini_maks(m, f.mini_maks(m))
        input("WCISNIJ ENTER ABY COFNĄĆ")
    elif option == '2':
        print_maks_maks(m, f.maks_maks(m))
        input("WCISNIJ ENTER ABY COFNĄĆ")
    elif option == '3':
        factor = float(input("Podaj współczynnik ostrożności: "))
        print_k_hurwicz(f.k_hurwicz(m, factor), factor)
        input("WCISNIJ ENTER ABY COFNĄĆ")
    elif option == '4':
        prob = []
